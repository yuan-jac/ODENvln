...
CUDA_VISIBLE_DEVICES=1,2 python3 -m torch.distributed.launch --node_rank 0 --nnodes=1  --master_port 29502 --nproc_per_node=${ngpus}  map_nav_src/main_nav.py $flag \
  --tokenizer bert \
	--bert_ckpt_file datasets/R2R/exprs_map/pretrain/cmt-vitbase-mlm.mrc.sap-init.lxmert-aug.speaker-new/ckpts/model_step_60000.pt \
  --eval_first
