...
class ReverieTextPathData(object):
    ...
    def get_traj_pano_fts(self, scan, path):
        ...
        for vp in path:
            ...
            self.cur_vp = vp
            self.global_semantic, self.global_position_x, self.global_position_y, self.global_mask, self.global_map, self.max_x, self.min_x, self.max_y, self.min_y, gridmap_pos_fts, target_patch_id = self.getGlobalMap(
                scan, vp)
...
class SoonTextPathData(ReverieTextPathData):
    def __init__(
            self, anno_files, img_ft_file, obj_ft_file, scanvp_cands_file, connectivity_dir,
            image_feat_size=2048, image_prob_size=0, angle_feat_size=4,
            obj_feat_size=None, obj_prob_size=None, max_objects=20,
            max_txt_len=100, in_memory=True, is_train=False, act_visited_node=False
    ):
        super().__init__(
