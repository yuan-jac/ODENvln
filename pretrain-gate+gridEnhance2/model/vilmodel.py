...
class GlocalTextPathCMT(BertPreTrainedModel):
    ...
    def forward(
        ...
        # 查看 grid_spatial_text、grid_fts 和 text_fts 的结构
        '''print("=== grid_spatial_text ===")
        print(f"type: {type(grid_spatial_text)}, len: {len(grid_spatial_text)}")
        if len(grid_spatial_text) > 0:
            print(f"  [0] type: {type(grid_spatial_text[0])}, shape: {getattr(grid_spatial_text[0], 'shape', 'N/A')}")

        print("=== grid_fts ===")
        print(f"type: {type(grid_fts)}, len: {len(grid_fts)}")
        if len(grid_fts) > 0:
            print(f"  [0] type: {type(grid_fts[0])}, shape: {getattr(grid_fts[0], 'shape', 'N/A')}")

        print("=== text_fts ===")
        print(f"type: {type(text_fts)}, len: {len(text_fts)}")
        if len(text_fts) > 0:
            print(f"  [0] type: {type(text_fts[0])}, shape: {getattr(text_fts[0], 'shape', 'N/A')}")'''

        grid_masks = [[] for b in range(batch_size)]
        max_cell_num = 0
        for b in range(batch_size):
            fused_grid_fts_b = self.fusion_module(grid_fts[b], grid_spatial_text[b])
