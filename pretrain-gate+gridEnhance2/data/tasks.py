...
from .common import pad_tensors, pad_sequence
...
class MlmDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        gst = inputs['grid_spatial_text']
        if isinstance(gst, list):
            gst = np.stack(gst, axis=0)
        output['grid_spatial_text'] = torch.from_numpy(gst).to(torch.float16)
...
def mlm_collate(inputs):
    ...
    for i in range(batch_size):
        gmap_pair_dists[i, :int(batch['gmap_lens'][i].item()), :int(batch['gmap_lens'][i].item())] = \
            batch['gmap_pair_dists'][i]
    ...
    batch["gridmap_pos_fts"] = torch.cat([torch.tensor(x).unsqueeze(0) for x in batch["gridmap_pos_fts"]], 0).to(
        torch.float32)
    return batch
