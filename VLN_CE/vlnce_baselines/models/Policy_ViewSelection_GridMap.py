...
from vlnce_baselines.models.utils import (
    angle_feature_with_ele, dir_angle_feature_with_ele, length2mask, pad_tensors, gen_seq_masks, get_angle_fts,
    get_angle_feature, get_point_angle_feature, calculate_vp_rel_pos_fts, calc_position_distance, pad_tensors_wgrad)
...
class GridMap(Net):
    ...
    def forward(self, mode=None,
                ...
                batch_grid_fts=None, batch_map_index=None, batch_gridmap_pos_fts=None, in_train=True):
        ...
        if mode == 'language':
            ...
            if in_train:
                batch_grid_fts = np.concatenate(
                    [self.grid_transforms_train(image).unsqueeze(0) for image in image_list], 0)
                batch_view_fts = np.concatenate(
                    [self.view_transforms_train(image).unsqueeze(0) for image in image_list], 0)
            else:
            ...
            for i in range(batch_size):
                ...
                self.global_fts[i], self.global_position_x[i], self.global_position_y[i], self.global_mask[i], \
                    self.global_map_index[i], self.max_x[i], self.min_x[i], self.max_y[i], self.min_y[
                    i], gridmap_pos_fts = self.getGlobalMap(i, position, heading, depth, grid_ft, image_list)
                batch_gridmap_pos_fts.append(gridmap_pos_fts)

            batch_gridmap_pos_fts = torch.cat(
                [torch.tensor(pos_fts).unsqueeze(0).cuda() for pos_fts in batch_gridmap_pos_fts], dim=0)
            ...
            for j in range(batch_size):
                ...
                cand_angle_fts = np.concatenate(
                    [np.expand_dims(get_angle_feature(batch_angles[j][k]), 0) for k in range(len(img_idxes))], axis=0)
            ...
            avg_pano_embeds = torch.sum(pano_embeds * pano_masks.unsqueeze(2), 1) / torch.sum(pano_masks, 1,
                                                                                              keepdim=True)
            ...
            for i in range(batch_size):
                ...
                for j in range(len(self.traj_map[i]) - 1, -1, -1):
                    rel_heading, rel_elevation, rel_dist = calculate_vp_rel_pos_fts(self.positions[i],
                                                                                    self.traj_map[i][j][0],
                                                                                    base_heading=self.headings[i],
                                                                                    base_elevation=0.)
                    rel_angles.append([rel_heading, rel_elevation])
                    rel_dists.append(
            ...
            logits = self.vln_bert('navigation',
                                   (lang_feats, lang_masks, batch_traj_img_embeds, batch_traj_step_ids,
                                    batch_traj_pos_fts, batch_traj_masks, vp_img_embeds, batch_vp_pos_fts,
                                    gen_seq_masks(batch_view_lens + 1), vp_nav_masks, batch_grid_fts, batch_map_index,
                                    batch_gridmap_pos_fts, candidate_lengths))
    ...
    def get_rel_position(self, depth_map, angle):
        ...
        if DATASET == 'R2R':
            depth_x = depth_y * (np.array([-6 / 7, -4 / 7, -2 / 7, 0., 2 / 7, 4 / 7, 6 / 7] * 7, np.float32) * math.tan(
                math.pi / 4.))
        elif DATASET == 'RxR':
            depth_x = depth_y * (np.array([-6 / 7, -4 / 7, -2 / 7, 0., 2 / 7, 4 / 7, 6 / 7] * 7, np.float32) * math.tan(
                math.pi * 79. / 360.))
    ...
    def image_get_rel_position(self, depth_map, angle):
        ...
        if DATASET == 'R2R':
            depth_x = depth_y * (np.array(([i / 128 for i in range(-128, 0)] + [i / 128 for i in range(0, 128)]) * 256,
                                          np.float32) * math.tan(math.pi / 4))
        elif DATASET == 'RxR':
            depth_x = depth_y * (np.array(([i / 128 for i in range(-128, 0)] + [i / 128 for i in range(0, 128)]) * 256,
                                          np.float32) * math.tan(math.pi * 79. / 360.))
        rel_x = depth_x * math.cos(angle) + depth_y * math.sin(angle)
        rel_y = depth_y * math.cos(angle) - depth_x * math.sin(angle)
        return rel_x, rel_y
    ...
    def getGlobalMap(self, batch_id, position, heading, depth, grid_ft, image_list):
        ...
        return self.global_fts[i], self.global_position_x[i], self.global_position_y[i], self.global_mask[i], \
            self.global_map_index[i], self.max_x[i], self.min_x[i], self.max_y[i], self.min_y[i], gridmap_pos_fts
