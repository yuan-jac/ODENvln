    ...
    def forward(self, mode=None,
        ...
        if mode == 'language':
            ...
            for i in range(batch_size):
                ...
                ### Traj Graph dist
                traj_pair_dists = np.zeros(
                    (candidate_lengths[i] + len(self.traj_map[i]), candidate_lengths[i] + len(self.traj_map[i])),
                    dtype=np.float32)
                for j in range(0, len(self.traj_map[i])):
                    ...
                    for k in range(candidate_lengths[i] - 1):
                        traj_pair_dists[j][len(self.traj_map[i]) + k] = traj_pair_dists[len(self.traj_map[i]) + k][j] = \
                            batch_distances[i][candidate_lengths[i] - 2 - k] + path_dist
            ...
            logits = self.vln_bert('navigation',
                                   (lang_feats, lang_masks, batch_traj_img_embeds, batch_traj_step_ids,
                                    batch_traj_pos_fts, batch_traj_masks, batch_traj_pair_dists, vp_img_embeds,
                                    batch_vp_pos_fts, gen_seq_masks(batch_view_lens + 1), vp_nav_masks,
                                    candidate_lengths))
